...
base_model='/home/rwkv/Peter/model/base/RWKV-x060-World-7B-v2.1-20240507-ctx4096.pth'
persona_domain_state='/home/rwkv/Peter/rwkv_graphrag/agents/persona_domain_states/RWKV-x060-World-7B-v2.1-20240507-ctx4096.pth.pth'
entity_types_state='/home/rwkv/Peter/rwkv_graphrag/agents/entity_type_extraction/RWKV-x060-World-7B-v2.1-20240507-ctx4096.pth.pth'
...
def format_input_and_entities(input_text, entity_types):
    ...
    with open('/home/rwkv/Peter/rwkv_graphrag/data/triple_input.jsonl', 'w', encoding='utf-8') as f:
...
persona=generate_persona_domain(model_path=base_model,states_file=persona_domain_state,input_text=input_text)
#time.sleep(10)
torch.cuda.empty_cache()
entity_types=generate_entity_types(model_path=base_model,states_file=entity_types_state,input_text=persona)
#time.sleep(10)
# torch.cuda.empty_cache()
# time.sleep(10)
# triple_input = '{\"input\": \"有个空空道人访道求仙，从大荒山无稽崖青埂峰下经过，忽见一大块石上字迹分明，编述历历，《石头记》是也。空空道人将《石头记》抄录下来，改名为《情僧录》。至吴玉峰题曰《红楼梦》。东鲁孔梅溪则题曰《风月宝鉴》。后因曹雪芹于悼红轩中披阅十载，增删五次，纂成目录，分出章回，则题曰《金陵十二钗》。姑苏乡宦甄士隐梦见一僧一道携无缘补天之石（通灵宝玉）下凡历练，又讲绛珠仙子为报神瑛侍者浇灌之恩追随神瑛侍者下世为人，以泪报恩。梦醒后，抱女儿英莲去看“过会”[2]。甄士隐结交并接济了寄居于隔壁葫芦庙内的胡州人氏贾化（号雨村）。某日，贾雨村造访甄士隐，无意中遇见甄家丫鬟娇杏，以为娇杏对其有意。中秋时节，甄士隐于家中宴请贾雨村，得知贾雨村的抱负后，赠银送衣以作贾雨村上京赴考之盘缠，第二天，贾雨村不辞而别便上路赴考。第二年元宵佳节当晚，甄家仆人霍启在看社火花灯时，不慎丢失了甄士隐唯一的女儿英莲。三月十五日，葫芦庙失火祸及甄家，落魄的甄士隐带家人寄居于如州岳丈封肃家中，后遇一僧一道，悟出《好了歌》真谛，随僧道而去。\"}, {\"entity_types\": [\"文学与神话\", \"历史背景\", \"影响分析\", \"改编过程\", \"角色贡献\", \"写作技巧评估\"]}'

triple_input=format_input_and_entities(input_text=input_text,entity_types=entity_types)
# print(triple_input)
# triples=generate_triples(model_path=base_model,states_file=triples_states_file,input_text=triple_input)
